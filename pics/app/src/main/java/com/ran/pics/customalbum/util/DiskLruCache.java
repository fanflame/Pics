...
public class DiskLruCache {
	...
	public static DiskLruCache openCache(Context context, File cacheDir, long maxByteSize) {
		...
		if (cacheDir.isDirectory() && cacheDir.canWrite() && UtilsCustomAlbum.getUsableSpace(cacheDir) > maxByteSize) {
	 ...
	 *            directory
	 */
	public static void clearCache(Context context, String uniqueName, String dir) {
		File cacheDir = getDiskCacheDir(context, uniqueName,dir);
		clearCache(cacheDir);
	}
	 ...
	 */
	public static File getDiskCacheDir(Context context, String uniqueName, String dir) {

		// Check if media is mounted or storage is built-in, if so, try and use
		// external cache dir
		// otherwise use internal cache dir
//		final String cachePath = Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)  ? Utils
//				.getExternalCacheDir(context).getPath() : context.getCacheDir().getPath();
		final String cachePath = dir;
	...
	public static String createFilePath(File cacheDir, String key) {
		try {
			...
			// but it will do for
			// this example
			return cacheDir.getAbsolutePath() + File.separator +key.substring(key.lastIndexOf("/"));
	...
	private boolean writeBitmapToFile(Bitmap bitmap, String file) throws IOException, FileNotFoundException {
		...
		try {
			out = new BufferedOutputStream(new FileOutputStream(file), UtilsCustomAlbum.IO_BUFFER_SIZE);
