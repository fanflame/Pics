...
public abstract class ImageWorker {
	...
	public void setDownloadDir(String downloadDir) {
		...
		if(mImageCache == null)
			mImageCache = new ImageCache(mContext, mContext.getClass().getSimpleName().toString(), downloadDir);
	}
	
	/**
	 * @return the mLoadImageCallback
	 */
	public LoadImageCallback getmLoadImageCallback() {
		return mLoadImageCallback;
	}

	/**
	 * @param mLoadImageCallback the mLoadImageCallback to set
	 */
	public void setLoadImageCallback(LoadImageCallback mLoadImageCallback) {
	...
	public void loadImage(Object data, ImageView imageView) {
		...
		if (bitmap != null) {
			...
			//通知图片加载进度
			if(null != mLoadImageCallback)
				mLoadImageCallback.loadComplate(imageView);
	...
	public Bitmap getImageBitmap(Object data, ImageView imageView){
		Bitmap bitmap = null;
		...
		if (bitmap == null && mImageCache != null && !mExitTasksEarly) {
		...
		if(null != bitmap && mLoadImageCallback != null)
			mLoadImageCallback.loadComplate(imageView);
		return bitmap;
	...
	private class BitmapWorkerTask extends AsyncTask<Object, Void, Bitmap> {
		...
		protected void onPostExecute(Bitmap bitmap) {
			...
			if (bitmap != null && imageView != null) {
				...
				if(null != mLoadImageCallback)
					mLoadImageCallback.LoadFail(imageView);
			}
			if(null != mLoadImageCallback && bitmap != null)
				mLoadImageCallback.loadComplate(imageView);
...
//		}
	...
	/**
	 * 
	* @ClassName: LoadImageCallback  
	* @Description: 图片加载进度通知
	* @author WQ 
	* @date 2014-2-14 上午8:39:26
	 */
	public static abstract class LoadImageCallback{
		public abstract void loadComplate(ImageView v);
		public void LoadFail(ImageView v){}
	} 
}
